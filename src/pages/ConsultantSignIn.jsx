...
import { logClient, logConsultant } from "../utils/api-interceptor"
import Loader from "../components/Loader"

const ConsultantSignIn = () => {
  ...
  const signInHandler = async (event) => {
    ...
    try {
      setLoading(true)
      const user = await logConsultant({ email, password })
      localStorage.setItem("token", JSON.stringify(user.data.access_token))
      setLoading(false)
      navigate("/platform/consultant/dashboard")
    } catch (error) {
      setLoading(false)
      console.log(error)
